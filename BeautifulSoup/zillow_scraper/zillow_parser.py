...
def parse_property_listing(article):
    ...
    property_detail = PropertyDetail(price, address, features, status, url)
...
def main() -> None:
    ...
    while html_file is not None:
        ...
        for article_element in property_article_elements:
            property_info = parse_property_listing(article_element)
    ...
    df.to_csv(f"zillow_properties_{timestamp}.csv", index=False)
